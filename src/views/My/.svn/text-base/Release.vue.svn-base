...
export default {
    ...
    mounted(){
        ...
        this.init();
        this.initPhone();
        this.timer = setInterval(this.beforeunloadHandler, 1000);
    },
    methods: {
        // 读取缓存
        init(){
            if(localStorage.params){
                let params = this.qs.parse(localStorage.params);
                // 基本信息
                this.carType = params['基本信息']['车辆类型'];
                this.vinCode = params['基本信息']['VIN码'];
                this.chexi = params['基本信息']['品牌车系'];
                this.chexing = params['基本信息']['具体车型'];
                this.licheng = params['基本信息']['表显里程'];
                this.firstCard = params['基本信息']['初次上牌'];
                this.carColor = params['基本信息']['车身颜色'];
                this.gearbox = params['基本信息']['变速箱'];
                this.pailiang = params['基本信息']['排量'];
                this.emission = params['基本信息']['排放标准'];
                this.hezhun = params['基本信息']['核准人数'];
                this.use = params['基本信息']['使用性质'];
                this.leaveFactory = params['基本信息']['出厂日期'];
                this.nianjian = params['基本信息']['年检到期日'];
                this.location = params['基本信息']['车辆所在地'];
                this.attribution = params['基本信息']['车辆归属地'];

                // 价格说明
                this.gerenjia = params['价格说明']['个人零售价'];
                this.cheshangjia = params['价格说明']['车商批发价'];
                this.transfer = params['价格说明']['含过户费'];

                // 车辆标签
                this.carLabelArr = params['车辆标签']['车辆标签'] == '' ? [] : params['车辆标签']['车辆标签'].split(',');
                // 车辆描述
                let mDesc = params['车辆描述']['车辆描述'].split(',');
                this.descCont = mDesc.pop().substring(0);
                this.descArr = mDesc;

                // 联系方式
                this.phone = params['联系方式']['联系电话'];


            };
            if(localStorage.waiguanImg){
                let waiguan = JSON.parse(localStorage.waiguanImg);
                this.waiguan = JSON.parse(JSON.stringify(waiguan));
            }
            if(localStorage.neishiImg){
                let neishi = JSON.parse(localStorage.neishiImg);
                this.neishi = JSON.parse(JSON.stringify(neishi));
            }
        },
        // 获取手机号
        initPhone(){
            ...
            .then( res => {
                if(res.data.data.verify_status == "1"){
        ...
        submit(){
            ...
            if(this.carType.length==0){
                ...
                this.$http({
                    method: 'post',
                    url: '/api/car_dealer_index.php?s=/Home/Sale/add',
                    data: this.qs.stringify({
                        openid: localStorage.openid,
                        fids: fids.toString(),
                        params: params
                ...
                .then( res => {
                    if(res.data.code == 0){
                        ...
                        localStorage.removeItem('neishiImg');
                        localStorage.removeItem('waiguanImg');
        ...
        beforeunloadHandler(e){
            ...
            localStorage.params = this.qs.stringify(params);
            localStorage.waiguanImg = JSON.stringify(this.waiguan);
            localStorage.neishiImg = JSON.stringify(this.neishi);
        }
    },
    // 销毁
    destroyed(){
